"""
rag_engine.py — RAG (Retrieval Augmented Generation) tizimi.
1) PDF matnini vektor bazaga (ChromaDB) saqlaydi.
2) Savolga mos parchani topadi.
3) Groq AI (Llama 3.3) orqali javob beradi.
   - Agar PDF dan parcha topilsa → shu asosida javob.
   - Agar topilmasa → umumiy bilimdan javob (sen so'ragan narsa!).
"""
import os
import chromadb
...
# Embedding modeli (matnni vektorga aylantiradi) — o'zbekcha yaxshi ishlaydi
embedding_model = SentenceTransformer('paraphrase-multilingual-MiniLM-L12-v2')

# ChromaDB — vektor baza (har bir foydalanuvchi uchun alohida)
...
def get_or_create_collection(user_id: int):
    """Har bir foydalanuvchi uchun alohida vektor to'plami."""
    collection_name = f"user_{user_id}"
    return chroma_client.get_or_create_collection(name=collection_name)


def add_document(user_id: int, chunks: list, doc_name: str):
    """PDF parchalarini foydalanuvchi bazasiga qo'shadi."""
...
def search_relevant_chunks(user_id: int, query: str, top_k: int = 3) -> list:
    """Savolga eng mos keluvchi parchalarni topadi."""
    collection = get_or_create_collection(user_id)
    ...
    for i, doc in enumerate(results['documents'][0]):
        chunks.append({
            "text": doc,
            "source": results['metadatas'][0][i].get("source", "noma'lum")
        })
    return chunks
...
def generate_answer(query: str, relevant_chunks: list) -> str:
    """
    JAVOB BERISH MANTIG'I (sen so'ragan asosiy narsa!):
    - Agar relevant_chunks BO'SH bo'lmasa → PDF asosida javob.
    - Agar BO'SH bo'lsa → umumiy AI javob (har qanday savolga).
    """
    if relevant_chunks:
        # ===== REJIM 1: PDF asosida javob =====
        context_text = "\n".join(
            [f"[{c['source']}]: {c['text']}" for c in relevant_chunks]
        )
        system_prompt = (
            "Sen StudyMate — do'stona o'quv yordamchisan. "
        ...
        # ===== REJIM 2: Umumiy AI javob (PDF yo'q bo'lsa ham) =====
        system_prompt = (
            "Sen StudyMate — do'stona, aqlli o'quv yordamchisan. "
...
def generate_summary(user_id: int) -> str:
    """PDF ning qisqacha xulosasini chiqaradi."""
    collection = get_or_create_collection(user_id)
    if collection.count() == 0:
